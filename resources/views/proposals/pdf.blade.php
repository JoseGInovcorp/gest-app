    ...
    <div class="container">
        ...
        <div class="company-header">
            ...
            <div class="document-info">
                <h1>PROPOSTA</h1>
                <div class="numero">#{{ $proposal->numero }}</div>
            </div>
        </div>
                ...
                @if($proposal->entity)
                    <div style="display: table; width: 100%;">
                        <div style="display: table-cell; width: 50%; padding-right: 10px; vertical-align: top;">
                            <p class="name">{{ $proposal->entity->name }}</p>
                            @if($proposal->entity->nif)
                                <p><strong>NIF:</strong> {{ $proposal->entity->nif }}</p>
                            @endif
                        ...
                        <div style="display: table-cell; width: 50%; padding-left: 10px; vertical-align: top;">
                            ...
                            @endif
                            @if($proposal->entity->email)
                                <p><strong>Email:</strong> {{ $proposal->entity->email }}</p>
                            @endif
                            @if($proposal->entity->phone)
                                <p><strong>Tel:</strong> {{ $proposal->entity->phone }}</p>
                            @endif
                        </div>
                    </div>
                @else
        ...
        <div class="proposal-details">
            <div class="detail-row">
                ...
                <div class="detail-value">{{ \Carbon\Carbon::parse($proposal->data_proposta)->format('d/m/Y') }}</div>
            </div>
            <div class="detail-row">
                <div class="detail-label">Válida até:</div>
                <div class="detail-value">{{ \Carbon\Carbon::parse($proposal->validade)->format('d/m/Y') }}</div>
            </div>
            <div class="detail-row">
                <div class="detail-label">Estado:</div>
                <div class="detail-value">
                    <span class="badge badge-{{ $proposal->estado }}">
                        {{ $proposal->estado === 'rascunho' ? 'Rascunho' : 'Fechado' }}
                    </span>
                </div>
            </div>
            ...
            <div class="detail-row">
                ...
                <div class="detail-value" style="white-space: pre-wrap;">{{ $proposal->observacoes }}</div>
            </div>
            @endif
        </div>
        ...
        <div class="items-section">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <div class="article-name">{{ $line->article->nome ?? 'N/A' }}</div>
                        @if($line->article && $line->article->referencia)
                            <div class="article-ref">Ref: {{ $line->article->referencia }}</div>
                        @endif
                    </td>
                    <td>{{ $line->entity->name ?? '-' }}</td>
                    <td class="text-right">{{ number_format($line->quantidade, 2, ',', '.') }}</td>
                    <td class="text-right">{{ number_format($line->preco_custo, 2, ',', '.') }} €</td>
        ...
        <div class="total-section">
            <div class="total-row">
                ...
                <span class="total-value">{{ number_format($proposal->valor_total, 2, ',', '.') }} €</span>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            <p style="margin-top: 10px; font-style: italic; color: #9ca3af;">
                Esta proposta é válida até {{ \Carbon\Carbon::parse($proposal->validade)->format('d/m/Y') }}
            </p>
        </div>
    </div>
</body>
